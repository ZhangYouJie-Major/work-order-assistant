...
"""
    ...
    async def generate_dml(
        ...
        Args:
            entities: 提取的实体信息
            prompt_template: DML 生成提示词模板

        Returns:
            DML 信息，格式:
            {
                "sql": "UPDATE orders SET status = 'cancelled' WHERE user_id = 12345",
...
请生成规范的 DML 语句，并评估风险，返回 JSON 格式:
{{
    "sql": "DML 语句",
    "operation_type": "INSERT" | "UPDATE" | "DELETE",
    "affected_tables": ["表名列表"],
    "estimated_rows": 预计影响行数,
    "risk_level": "low" | "medium" | "high",
    "description": "操作说明",
    "conditions": {{"WHERE 条件字段": "条件值"}}
}}
"""
