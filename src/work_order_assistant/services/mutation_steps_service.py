...
class MutationStepsService:
    ...
    async def match_config_by_content(
        ...
        system_prompt = "你是一个工单分类专家。请根据工单内容，从以下配置中选择最匹配的一个。你需要同时评估工单内容的质量和匹配度。"
...
**重要评估标准**：
1. 工单内容是否清晰、完整、合理
2. 是否包含无意义的词汇或表述不清的部分
3. 关键信息（如订单号、操作类型）是否明确
4. 如果工单内容包含无意义词汇、表述不清或信息不完整，应**显著降低**置信度

**置信度评分指导**：
- 1.0: 工单内容非常清晰、完整，完全符合配置描述，无任何歧义或无意义词汇
- 0.8-0.9: 工单内容清晰，基本符合配置描述，信息完整
- 0.6-0.7: 工单内容可以理解，但有部分不清晰或不完整
- 0.4-0.5: 工单内容包含无意义词汇或表述不清，但关键信息可识别
- <0.4: 工单内容混乱或无法理解
...
{{
    ...
    "reasoning": "匹配理由，需要明确说明：1) 工单内容的质量评估 2) 为什么匹配该配置"
}}
"""
